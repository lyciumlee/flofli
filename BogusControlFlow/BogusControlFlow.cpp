...
struct BogusControlFlow : public FunctionPass {
  ...
  void bogus(Function &F) {
    ...
    if (ObfProbRate < 0 || ObfProbRate > 100) {
    errs() << "bcf: Incorrect value," << " probability rate set to default value: " << defaultObfRate << " \n";
      ObfProbRate = defaultObfRate;
    }
    errs() << "bcf: How many times: " << ObfTimes << "\n" ;
    if (ObfTimes <= 0) {
    errs() << "bcf: Incorrect value," << " must be greater than 1. Set to default: " << defaultObfTime << " \n" ;
    ...
    do {
      ...
      for (Function::iterator i = F.begin(); i != F.end(); ++i) {
        ...
        errs() << "bcf: Iterating on the Function's Basic Blocks\n" ;

      while (!basicBlocks.empty()) {
        NumBasicBlocks++;
        // Basic Blocks' selection
        if ((int)llvm::cryptoutils->get_range(100) <= ObfProbRate) {
          errs() << "bcf: Block " << NumBasicBlocks << " selected. \n";
          ...
          errs() << "bcf: Block " << NumBasicBlocks << " not selected.\n" ;
        }
  ...
  virtual void addBogusFlow(BasicBlock *basicBlock, Function &F) {
    ...
    errs() << "bcf: First and original basic blocks: ok\n";
      ...
      if (i->isBinaryOp()) { // binary instructions
        ...
        if (opcode == Instruction::FAdd || opcode == Instruction::FSub ||
          ...
          for (int random = (int)llvm::cryptoutils->get_range(10); random < 10;
            ...
            {
                Value *zero = ConstantFP::getZeroValueForNegation(i->getOperand(0)->getType());
                op = BinaryOperator::Create(Instruction::FSub, zero, (Value *)i->getOperand(0), (const Twine)*var, (Instruction *)&*i);
  ...
  bool doF(Module &M) {
    ...
    for (std::vector<Instruction *>::iterator i = toEdit.begin();
      ...
      opY = new LoadInst(x->getType()->getPointerElementType(),(Value *)y, "", (*i));
